...
namespace EvidenciasSQA.Editor.Configuration
{
    ...
    ///
    /// The format is the one produced by the old EvidenciasSQA IniFile subsystem:
    ...
    /// </summary>
    internal class EvidenciasSQAEditorObjectValueConverter : ValueConverterBase<object>
    {
        private static readonly ILog LOG = LogManager.GetLogger(typeof(EvidenciasSQAEditorObjectValueConverter));
        public override object ConvertFromString(string raw, object defaultValue = default)
        {
            ...
            throw new SecurityException($"Unexpected type in .ini file detected, maybe vulnerability attack? Suspicious information: {raw}");
        }
        ...
        public override string ConvertToString(object value)
        {
            ...
            // Build assembly qualifier, abbreviated for EvidenciasSQA assemblies
            var assemblyName = type.Assembly.FullName ?? string.Empty;
            if (assemblyName.StartsWith("EvidenciasSQA", StringComparison.OrdinalIgnoreCase))
            {

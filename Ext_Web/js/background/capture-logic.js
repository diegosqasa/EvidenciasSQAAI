...
// Import estático obligatorio: el import() dinámico está prohibido en
// ServiceWorkerGlobalScope (spec HTML), el fast-path fallaba siempre.
import * as CaptureEngine from './CaptureEngine.js';
...
function isPdfPageTab(tab) {
    ...
    return /\.pdf($|[?#])/i.test(tab.url) || /application\/pdf/i.test(tab.url);
}
...
export async function executeCapture(tab, actionName) {
    ...
    // PDF local (viewer OOPIF sin scroll DOM): la extensión no puede
    // auto-desplazarse por todo el documento, así que se informa al usuario
    // y se cambia el modo de captura a "solo la pantalla visible".
    if (actionName === "captureAllPageScreenshot" && isPdfPageTab(tab)) {
        const pdfNotice = "La extensión detectó un archivo PDF local sin scroll DOM. No se puede desplazar automáticamente por el documento. Captura en modo solo la pantalla visible.";
        updateCaptureStatus({
            active: true,
            mode: 'visible',
            progress: 10,
        ...
        console.warn('[capture-logic] ' + pdfNotice);
        actionName = "captureVisibleOnly";
    }
    ...
    if (actionName === "captureAllPageScreenshot" || actionName === "captureVisibleOnly") {
        try {
            ...
            const isServiceWorkerError = err.message && (
                err.message.includes('Service Worker') ||
                err.message.includes('ServiceWorkerGlobalScope')
            );

	...
	if (!window.hasInjectedContentScript || contextInvalidated) {
		...
		var capturex_com_saveAction = {
			...
			captureAllPageScreenshot: function (nowTop = 0) {
				// PDF local (viewer OOPIF sin scroll DOM): no se puede auto-desplazar
				// el documento. Se informa al usuario y se captura solo lo visible.
				if (document.contentType === 'application/pdf' || (/\.pdf($|[?#])/i).test(window.location.href)) {
					console.info('[SQA] PDF detectado sin scroll DOM — cambiando a captura de la pantalla visible');
					capturex_com_saveAction.showTip('PDF detectado: no se puede desplazar el documento. Captura en modo solo pantalla visible');

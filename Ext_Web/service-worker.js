...
const PDF_RENDER_DOC_URL = 'pdf-render.html';

// Acumula chunks del PNG renderizado por el offscreen document pdf-render
    ...
    const handlers = {
      'themeChanged': () => {
        ...
        "pdfCaptureRequest": () => {
            ...
            if (targetTab) {
                ...
                chrome.tabs.get(message.tabId, (tab) => {
                    if (!chrome.runtime.lastError && tab) startPdfCapture(tab);
                });
                sendResponse({ started: true });
            } else {
        ...
        "pdfRenderChunk": () => {
            ...
            if (message.index === 0 && !entry.total) entry.total = message.total;
            if (entry.received === entry.total && allChunksPresent(entry.chunks)) {
                const fullData = 'data:image/png;base64,' + entry.chunks.join('');
                finishPdfCaptureCleanup(tabId);
                const activeTab = entry.tab;
                touchHeartbeat();
                processFinalImage(fullData, activeTab);
            }
        ...
        "pdfRenderError": () => {
            ...
            if (tabId) sendPdfVisibleFallback(tabId);
            sendResponse({ ok: true });
        }
    };
...
async function ensurePdfRenderDoc() {
    const docUrl = chrome.runtime.getURL(PDF_RENDER_DOC_URL);
    if (typeof chrome.runtime.getContexts === 'function') {
        const contexts = await chrome.runtime.getContexts({ contextTypes: ['OFFSCREEN_DOCUMENT'], documentUrls: [docUrl] });
        if (contexts.length > 0) return true;
    }
    await chrome.offscreen.createDocument({
        url: PDF_RENDER_DOC_URL,
        reasons: ['WORKERS'],
        justification: 'Renderizado full-page de documentos PDF con pdf.js para captura de evidencias.'
    });
    return true;
}

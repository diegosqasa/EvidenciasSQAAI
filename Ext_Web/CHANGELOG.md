...
### v3.8.1 — 2026-08-15
**Fix: Fast-path debugger inoperante en el Service Worker**
- **Causa raíz**: `capture-logic.js` cargaba `CaptureEngine.js` con import dinámico (`import('./CaptureEngine.js')`), que está **prohibido en `ServiceWorkerGlobalScope`** por la especificación HTML (w3c/ServiceWorker#1356). El fast-path con `chrome.debugger` fallaba SIEMPRE y la extensión caía al content script (más lento, con stitching), logueando el warning `[capture-logic] Fast-path debugger falló, usando content script`.
- **Fix**: import estático `import * as CaptureEngine from './CaptureEngine.js'` (el service worker ya está registrado como `"type": "module"` en `manifest.json`, que sí permite imports estáticos). Se eliminó el `withTimeout(import(...))` del fast-path.
- **Extra**: el catch del fast-path ahora también clasifica como error esperado (sin warning) los mensajes que contienen `ServiceWorkerGlobalScope`, para no loguear ruido si aparece de nuevo en otro contexto.

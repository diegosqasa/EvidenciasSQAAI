...
private string CreateOutputFilePath()
        {
            // Direct save flow: all captures are stored in this fixed QA evidence folder
            string qaOutputFilePath = @"C:\Users\DiegoFernandoMachado\CapturasQA";
            try
            {
                Directory.CreateDirectory(qaOutputFilePath);
                return qaOutputFilePath;
            }
            catch (Exception ex)
            {
                Log.Warn(ex);
            ...
            {
                string pafOutputFilePath = Path.Combine(Application.StartupPath, @"..\..\Documents\Pictures\EvidenciasSQAs");
...
public void ValidateAndCorrectOutputFileAsFullpath()
        {
            ...
            {
                OutputFileAsFullpath = Path.Combine(CreateOutputFilePath(), "dummy.png");
            }
        }
...
if (string.IsNullOrEmpty(OutputFileAsFullpath))
            {
                OutputFileAsFullpath = EvidenciasSQAEnvironment.IsPortable
                    ? Path.Combine(Application.StartupPath, @"..\..\Documents\Pictures\EvidenciasSQAs\dummy.png")
                    : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "dummy.png");
            }

            // Direct save flow (forced): every capture is saved automatically as PNG in the
            // fixed QA evidence folder, without destination picker, quality prompt or save-as dialog.
            OutputFilePath = CreateOutputFilePath();
            OutputFileAsFullpath = Path.Combine(OutputFilePath, "dummy.png");
            OutputFileFilenamePattern = "Captura_${capturetime:d\"yyyy-MM-dd_HHmmss\"}";
            OutputFileFormat = OutputFormat.png;
            OutputFilePromptQuality = false;
            OutputFileAllowOverwrite = false;
            OutputDestinations = new List<string> { nameof(WellKnownDestinations.FileNoDialog) };

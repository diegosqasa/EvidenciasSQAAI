...
namespace EvidenciasSQA.Base.Core
{
    ...
    public static class PluginUtils
    {
        ...
        /// <summary>
        /// Helper method to add a plugin MenuItem to the EvidenciasSQA context menu
        /// </summary>
        /// <param name="item">ToolStripMenuItem</param>
        public static void AddToContextMenu(ToolStripMenuItem item)
        {
            // Direct save flow: the tray context menu is locked to the capture options and
            // exit only, so plugins are not allowed to add their own menu entries anymore.
            Log.Debug("AddToContextMenu is disabled in the direct save flow: " + item.Name);
        }
    }
}
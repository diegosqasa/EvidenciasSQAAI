...
{
    [IniSection("Core")]
    [Description("EvidenciasSQA core configuration")]
    public interface ICoreConfiguration : IIniSection, INotifyPropertyChanged, IAfterLoad, IBeforeSave
    {
        ...
        [Description("If the target file already exists True will make EvidenciasSQA always overwrite and False will display a 'Save-As' dialog.")]
        [DefaultValue(true)]
        ...
        [Description("The wav-file to play when a capture is taken, loaded only once at the EvidenciasSQA startup")]
        ...
        [Description("Optional command to execute on a temporary PNG file, the command should overwrite the file and EvidenciasSQA will read it back. Note: this command is also executed when uploading PNG's!")]
        [DefaultValue("")]
        string OptimizePNGCommand { get; set; }

        [Description("Arguments for the optional command to execute on a PNG, {0} is replaced by the temp-filename from EvidenciasSQA. Note: Temp-file is deleted afterwards by EvidenciasSQA.")]
        [DefaultValue("\"{0}\"")]
        string OptimizePNGCommandArguments { get; set; }

        [Description("Version of EvidenciasSQA which created this .ini")]

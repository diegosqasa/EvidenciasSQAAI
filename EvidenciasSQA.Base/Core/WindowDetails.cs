...
using EvidenciasSQA.Base.Interop;
using log4net;
...
namespace EvidenciasSQA.Base.Core
{
    ...
    /// but a LOT of changes/enhancements were made to adapt it for EvidenciasSQA.
    ///
    ...
    public class WindowDetails : IEquatable<WindowDetails>
    {
        ...
        private static readonly IList<string> SystemWindowClasses = new List<string>(new[]
        {
            "Progman",           // Program Manager (Desktop)
            "WorkerW",           // Worker Window (Desktop)
            "Shell_TrayWnd",     // Taskbar
            "Shell_SecondaryTrayWnd", // Secondary Taskbar
            "DV2ControlHost",    // Windows 11 Taskbar
            "Windows.UI.Core.CoreWindow", // UWP App Frame
            "ApplicationFrameWindow",   // UWP App Frame
            "Button",            // Generic button
            "Dwm",               // DWM Window
            "MS-SDIa"            // Legacy SDI app
        });
        ...
        static WindowDetails()
        {
            try
            {
                ...
                {
                    // .NET 9 (consolidacion): COMWrapper (RealProxy/remoting) ya no existe.
                    // Creacion COM directa vía el CLSID declarado en IAppVisibility.
                    Type comType = Type.GetTypeFromCLSID(new Guid("7E5FE3D9-985F-4908-91F9-EE19F9FD1514"));
                    if (comType != null)
                    {
                        AppVisibility = (IAppVisibility)Activator.CreateInstance(comType);
                    }
                }
            }
            catch (Exception ex)
        ...
        private bool CanFreezeOrUnfreeze(string titleOrProcessname)
        {
            ...
            if (titleOrProcessname.ToLower().Contains("evidenciassqa"))
            {
                return false;
            }
        ...
        public static WindowDetails GetActiveWindow()
        {
            ...
            if (hWnd != IntPtr.Zero)
            {
                ...
                {
                    // Foreground window is our own app (MainForm, CaptureForm, SharedMessageWindow)
                    // Search for the last valid user window
                    return GetLastValidUserWindow();
                }
                ...
                {
                    // Search for the last valid user window instead of returning Desktop
                    return GetLastValidUserWindow();
                }

                // Check if it's a system window we should ignore
                if (IsSystemWindow(activeWindow))
                {
                    return GetLastValidUserWindow();
                }
            ...
            // No foreground window, try to find last valid user window
            return GetLastValidUserWindow();
        }

        /// <summary>
        /// Checks if a window is a system window that should not be captured as "active window"
        /// </summary>
        private static bool IsSystemWindow(WindowDetails window)
        {
            ...
            return SystemWindowClasses.Contains(className);
        }

        /// <summary>
        /// Gets the last valid user window from visible windows, excluding system windows and our own app windows
        /// </summary>
        private static WindowDetails GetLastValidUserWindow()
        {
            try
            {
                ...
                // First pass: find the most recently active valid window
                foreach (var window in visibleWindows)
                {
                    ...
                    // Valid user window found
                    if (Log.IsDebugEnabled)
                    {
                        Log.Debug("[GetLastValidUserWindow] Found valid window: " + window.Text + " (" + window.ClassName + ") HWND=" + window.Handle);
                    }
                    return window;
                }
                
                Log.Warn("[GetLastValidUserWindow] No valid user window found among visible windows");
            }
            catch (Exception ex)
            {
                Log.Error("[GetLastValidUserWindow] Error searching for valid window: " + ex.Message);
            }

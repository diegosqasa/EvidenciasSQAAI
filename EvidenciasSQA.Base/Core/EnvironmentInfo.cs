...
namespace EvidenciasSQA.Base.Core
{
    ...
    public static class EnvironmentInfo
    {
        ...
        public static string GetEvidenciasSQAVersion(bool shortVersion = false)
        {
            var executingAssembly = Assembly.GetExecutingAssembly();

            // Use assembly version
            string evidenciassqaVersion = executingAssembly.GetName().Version.ToString();
            ...
            if (!string.IsNullOrEmpty(assemblyFileVersionAttribute?.Version))
            {
                ...
                evidenciassqaVersion = assemblyFileVersion.ToString(2);
                try
                {
                    evidenciassqaVersion = assemblyFileVersion.ToString(3);
                }
                catch (Exception)
                {
                    // Ignore
            ...
            if (!shortVersion)
            {
                ...
                {
                    evidenciassqaVersion = informationalVersionAttribute.InformationalVersion;
                }
            }

            return evidenciassqaVersion.Replace("+", " - ");
        }

        public static string EnvironmentToString(bool newline)
        {
            StringBuilder environment = new();
            environment.Append("Software version: " + GetEvidenciasSQAVersion());
            ...
            environment.Append(string.Join(",", SimpleServiceProvider.Current.GetAllInstances<IEvidenciasSQAPlugin>().Select(plugin => plugin.Name)));

...
using EvidenciasSQA.Base.Core.Enums;
using EvidenciasSQA.Base.Core.FileFormatHandlers;
using EvidenciasSQA.Base.Help;
using EvidenciasSQA.Base.Interfaces;
using EvidenciasSQA.Base.Interfaces.Ocr;
using EvidenciasSQA.Configuration;
using EvidenciasSQA.Controls;
using EvidenciasSQA.Destinations;
using EvidenciasSQA.Editor;
using EvidenciasSQA.Editor.Destinations;
using EvidenciasSQA.Editor.Drawing;
using EvidenciasSQA.Editor.Forms;
using EvidenciasSQA.Helpers;
using EvidenciasSQA.Plugin.Win10;
using EvidenciasSQA.Processors;
using log4net;
...
{
    /// <summary>
    /// This is the MainForm, the shell of EvidenciasSQA
    /// </summary>
    public partial class MainForm : BaseForm, IEvidenciasSQAMainForm, ICaptureHelper, IProvideDeviceDpi
    {
        ...
        /// <summary>
        /// Application entry-point, called from <see cref="EvidenciasSQAMain"/> after the
        /// <see cref="IniConfigRegistry"/> has been set up and command-line arguments
        /// have been parsed.
        /// </summary>
        public static MainForm Start(CommandLineOptions options)
        {
            try
            {
                // Fix for Bug 2495900, Multi-user Environment
                // check whether there's an local instance running already
                _applicationMutex = ResourceMutex.Create("F48E86D3-E34C-4DB7-8F8F-9A0EA55F0D08", "EvidenciasSQA", false);
                ...
                if (isAlreadyRunning)
                {
                    ...
                    else
                    {
                        ...
                        foreach (Process evidenciassqaProcess in Process.GetProcessesByName("evidenciassqa"))
                        {
                            try
                            {
                                instanceInfo.Append(index++ + ": ").AppendLine(Kernel32Api.GetProcessPath(evidenciassqaProcess.Id));
                                if (currentProcessId == evidenciassqaProcess.Id)
                                {
                        ...
                        // A dirty fix to make sure the message box is visible as a EvidenciasSQA window on the taskbar
                        using Form dummyForm = new Form
                        {
                            Icon = EvidenciasSQAResources.GetEvidenciasSQAIcon(),
                            ShowInTaskbar = true,
                ...
                // From here on we continue starting EvidenciasSQA
                Application.EnableVisualStyles();
                Application.SetCompatibleTextRenderingDefault(false);

                Application.ApplicationExit += Application_ApplicationExit;

                // Consolidacion .NET 9: ya no hay Application.Run propio ni interop WinForms<->WPF.
                // El message pump es el Dispatcher de la Application WPF unificada (App.xaml.cs);
                // este metodo solo valida el arranque (mutex, copydata, restart) y entrega el
                // MainForm oculto para que App.OnStartup lo muestre junto a la ventana del visor.
                return new MainForm(options);
            }
            catch (Exception ex)
            {
                Log.Error("Exception in startup.", ex);
                EvidenciasSQAMain.Application_ThreadException(ActiveForm, new ThreadExceptionEventArgs(ex));
                return null;
            }
        }
        ...
        public MainForm(CommandLineOptions options)
        {
            ...
            SimpleServiceProvider.Current.AddService<IEvidenciasSQAMainForm>(this);
            ...
            // Premium tray menu: Segoe UI Variable typography, linear vector icons and SQA header
            TrayMenuStyle.ApplyTo(contextMenu, DeviceDpi);
            
            // Increase tray menu icon size to 32x32 for better visibility
            contextMenu.ImageScalingSize = new Size(32, 32);

            // Windows 11 rounded corners on the menu surface (region follows the DWM shadow)
            contextMenu.Opened += DropDownOpened_Round;
            MakeRoundedOnOpen(contextmenu_capturefullscreen.DropDown);
            MakeRoundedOnOpen(contextmenu_capturewindowfromlist.DropDown);

            notifyIcon.Icon = CreateTrayIcon();
            this.Icon = notifyIcon.Icon; // Icono de la ventana = icono del tray
            ...
            // This forces the registration of all destinations inside EvidenciasSQA itself.
            RegisterInternalDestinations();
            // This forces the registration of all processors inside EvidenciasSQA itself.
            ...
            // Añade la opción "Abrir Visor" al menú del tray (lanza el visor de evidencias).
            AddOpenVisorMenuItem();
            ...
            // Set the EvidenciasSQA icon visibility depending on the configuration. (Added for feature #3521446)
            ...
            _copyData.Channels.Add("EvidenciasSQA");
            ...
            // Make EvidenciasSQA use less memory after startup
        ...
        /// <summary>
        /// Applies the rounded Windows 11 region to a ToolStripDropDown when it opens.
        /// </summary>
        private static void MakeRoundedOnOpen(ToolStripDropDown dropDown)
        {
            ...
            dropDown.Opened -= DropDownOpened_Round;
            dropDown.Opened += DropDownOpened_Round;
            dropDown.SizeChanged -= DropDownSizeChanged_Round;
            dropDown.SizeChanged += DropDownSizeChanged_Round;
        }

        private static void DropDownOpened_Round(object sender, EventArgs e)
        {
            if (sender is ToolStripDropDown dropDown)
            {
                TrayMenuStyle.ApplyRoundedRegion(dropDown);
            }
        }

        private static void DropDownSizeChanged_Round(object sender, EventArgs e)
        {
            if (sender is ToolStripDropDown dropDown)
            {
                TrayMenuStyle.ApplyRoundedRegion(dropDown);
        ...
        /// <summary>
        /// Crea el icono de la bandeja del sistema desde Media\SQA1.ico o Media\SQA1.png.
        /// </summary>
        private static Icon CreateTrayIcon()
        {
            string[] icoCandidates =
            {
                Path.Combine(AppContext.BaseDirectory, "Media", "SQA1.ico"),
                Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Media", "SQA1.ico"),
                Path.Combine(AppContext.BaseDirectory, "icons", "applicationIcon", "icon.ico"),
                Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "icons", "applicationIcon", "icon.ico")
            };
            ...
            foreach (string path in icoCandidates)
            {
                try
                {
                    if (File.Exists(path))
                    {
                        using (Icon fromFile = new Icon(path, 32, 32))
                        {
            ...
            try
            {
                ...
                foreach (string path in pngCandidates)
                {
                    if (!File.Exists(path))
                    {
                        continue;
                    ...
                    using (var source = new Bitmap(path))
                    {
                        ...
                        using (var canvas = new Bitmap(size, size, PixelFormat.Format32bppArgb))
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    // Clone para desvincular el handle de Icon.FromHandle
                                    // (DestroyIcon libera el original; el clon es dueño).
            ...
            {
                // Fallback al icono embebido
            }

            return EvidenciasSQAResources.GetEvidenciasSQAIcon();
        }
        ...
        private void MultiScreenDropDownOpening(object sender, EventArgs e)
        {
            ...
            captureScreenItem.ShortcutKeyDisplayString = HotkeyManager.GetLocalizedHotkeyStringFromString(_conf.FullscreenHotkey);
            ...
            MakeRoundedOnOpen(captureScreenMenuItem.DropDown);
        }
        ...
        private void Contextmenu_CaptureWindow_Click(object sender, EventArgs e)
        {
            Dispatcher.CurrentDispatcher.BeginInvoke(() =>
            {
                CaptureHelper.CaptureWindow(false);
            });
        }

        private void MenuGrabarPantalla_Click(object sender, EventArgs e)
        {
            Dispatcher.CurrentDispatcher.BeginInvoke(() =>
            {
                CaptureHelper.CaptureWindow(false);
        ...
        private void MenuExtraerTexto_Click(object sender, EventArgs e)
        {
            Dispatcher.CurrentDispatcher.BeginInvoke(() =>
            {
        ...
        /// <summary>
        /// Context menu entry "Support EvidenciasSQA"
        /// </summary>
        /// <param name="sender">object</param>
        ...
        private void Contextmenu_DonateClick(object sender, EventArgs e)
        {
            Dispatcher.CurrentDispatcher.BeginInvoke(() =>
            {
                Process.Start("https://evidenciassqa.com/support/?version=" + EnvironmentInfo.GetEvidenciasSQAVersion(true));
            });
        }

        /// <summary>
        /// Inserta la opción "Abrir Visor" entre las capturas y la salida del
        /// menú del tray (después del separador de capturas). Idempotente por diseño:
        /// se invoca una única vez desde el constructor.
        /// </summary>
        private void AddOpenVisorMenuItem()
        {
            var openVisorItem = new ToolStripMenuItem("Abrir Visor");
            openVisorItem.Click += Contextmenu_OpenVisorClick;

            int insertIndex = contextMenu.Items.IndexOf(toolStripListCaptureSeparator) + 1;
            if (insertIndex > 0)
            {
                contextMenu.Items.Insert(insertIndex, openVisorItem);
            }
            else
            {
                contextMenu.Items.Add(openVisorItem);
            }
        }

        /// <summary>
        /// Muestra la ventana WPF del Visor dentro del mismo proceso (consolidacion
        /// .NET 9: ya no se lanza EvidenciasSQA.Wpf.exe ni se usan named pipes) y
        /// notifica al visor que cargue la última captura persistida (RestoreViewerRequested).
        /// </summary>
        private void Contextmenu_OpenVisorClick(object sender, EventArgs e)
        {
            Dispatcher.CurrentDispatcher.BeginInvoke(() =>
            {
        ...
        /// <summary>
        /// The "About EvidenciasSQA" entry is clicked
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        private void NotifyIconClickTest(object sender, MouseEventArgs e)
        {
            ...
            {
                // Right button: el systema muestra el context menu automáticamente
                // (contextMenu del NotifyIcon) → menú de capturas.
                return;
            }

            // Clic izquierdo: se muestra SIEMPRE el menú de capturas (igual que el clic
            // derecho). El visor solo se abre con "Abrir Visor" del menú; los clics del
            // icono nunca disparan RaiseRestoreViewerRequested.
            NotifyIconClick(ClickActions.SHOW_CONTEXT_MENU);
        }
        ...
        public void Exit()
        {
            ...
            // Consolidacion .NET 9: el message pump es el Dispatcher de la Application
            // WPF (ShutdownMode.OnExplicitShutdown). Application.Exit() (WinForms) solo
            // cierra formularios WinForms y NO termina el proceso; sin Shutdown() el
            // dispatcher WPF sigue vivo y la app queda corriendo en segundo plano tras
            // "Salir" del tray. Se marca la salida real (para que el Closing del visor
            // no se cancele) y se apaga la Application WPF explícitamente.
            try
            {
                VisorWindowManager.SetQuitting();
            ...
            {
                Log.Error("Error shutting down WPF application!", e);
            }
        }

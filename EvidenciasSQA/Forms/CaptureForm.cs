...
namespace EvidenciasSQA.Forms
{
    ...
    public sealed partial class CaptureForm : AnimatingForm
    {
        ...
        private static readonly Brush SelectionOverlayBrush;
        private static readonly Pen SelectionOverlayPen;
        private static CaptureForm _currentForm;
        private static readonly Brush BackgroundBrush;

        /// <summary>
        /// Colores corporativos para el overlay de selección.
        /// </summary>
        private static readonly Color CorporateBlue = Color.FromArgb(0x00, 0x2B, 0x55);      // #002B55
        private static readonly Color CorporateBlueMedium = Color.FromArgb(0x00, 0x40, 0x80); // #004080
        private static readonly Color CorporateOrange = Color.FromArgb(0xFF, 0x6B, 0x00);     // #FF6B00
        private static readonly int OverlayAlpha = 50; // 50/255 ≈ 20% opacidad
        private static readonly int BorderAlpha = 180; // 180/255 ≈ 70% opacidad
        ...
        static CaptureForm()
        {
            SelectionOverlayBrush = new SolidBrush(Color.FromArgb(OverlayAlpha, CorporateBlueMedium));
            SelectionOverlayPen = new Pen(Color.FromArgb(BorderAlpha, CorporateOrange), 2);

            using (Image backgroundForTransparency = EvidenciasSQAResources.GetImage("Checkerboard.Image"))
            {
            ...
            {
                SelectionOverlayBrush?.Dispose();
                SelectionOverlayPen?.Dispose();
        ...
        public CaptureForm(ICapture capture, List<WindowDetails> windows)
        {
            ...
            Text = @"EvidenciasSQA capture form";
        ...
        private void OnPaint(object sender, PaintEventArgs e)
        {
            ...
            if (_mouseDown || _captureMode == CaptureMode.Window || IsAnimating(_windowAnimator))
            {
                ...
                //} else {
                graphics.FillRectangle(SelectionOverlayBrush, fixedRect);
                //}
                graphics.DrawRectangle(SelectionOverlayPen, fixedRect);
                ...
                using (Font rulerFont = new Font(FontFamily.GenericSansSerif, 8))
                {
                    ...
                    using Brush bgBrush = new SolidBrush(Color.FromArgb(200, CorporateBlueMedium.R, CorporateBlueMedium.G, CorporateBlueMedium.B));
                    using Pen rulerPen = new Pen(CorporateOrange);
                ...
                if (newSize >= 4)
                {
                    ...
                    graphics.DrawString(sizeText, newSizeFont, new SolidBrush(CorporateOrange), sizeLocation);
            ...
            else
            {
                if (!IsTerminalServerSession)
                {
                    using (Pen pen = new Pen(CorporateBlueMedium))
                    {

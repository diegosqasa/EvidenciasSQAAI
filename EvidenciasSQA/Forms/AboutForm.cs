...
namespace EvidenciasSQA.Forms
{
    ...
    public sealed partial class AboutForm : AnimatingBaseForm
    {
        ...
        public AboutForm()
        {
            ...
            lblTitle.Text = $@"EvidenciasSQA {EnvironmentInfo.GetEvidenciasSQAVersion()} {(EvidenciasSQAEnvironment.IsPortable ? " Portable" : "")} ({OsInfo.Bits} bit) {(coreConfiguration.IsBetaTester ? "-IsBetaTester-":"")}";
            ...
            try
            {
                switch (keyData)
                {
                    ...
                    case Keys.L:
                        try
                        {
                            if (File.Exists(EvidenciasSQAMain.LogFileLocation))
                            {
                                using (Process.Start("\"" + EvidenciasSQAMain.LogFileLocation + "\""))
                                {
                            ...
                            {
                                MessageBox.Show(@"EvidenciasSQA can't find the logfile, it should have been here: " + EvidenciasSQAMain.LogFileLocation);
                            }
                        }
                        catch (Exception)
                        {
                            MessageBox.Show(@"Couldn't open the evidenciassqa.log, it's located here: " + EvidenciasSQAMain.LogFileLocation, @"Error opening evidenciassqa.log",
                    ...
                    case Keys.I:
                        try
                        {
                            using (Process.Start("\"" + EvidenciasSQAEnvironment.ConfigLocation + "\""))
                            {
                            }
                        }
                        catch (Exception)
                        {
                            MessageBox.Show(@"Couldn't open the evidenciassqa.ini, it's located here: " + EvidenciasSQAEnvironment.ConfigLocation, @"Error opening evidenciassqa.ini",
                                MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                        }

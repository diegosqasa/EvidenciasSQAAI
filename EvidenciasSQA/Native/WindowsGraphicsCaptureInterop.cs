...
using EvidenciasSQA.Native.DirectX;
using log4net;
using Windows.Graphics.Capture;
using Windows.Graphics.DirectX;
using Windows.Graphics.DirectX.Direct3D11;

namespace EvidenciasSQA.Native
{
    /// <summary>
    ...
    internal partial class WindowsGraphicsCaptureInterop
    {
        ...
        {
            var interop = GetActivationFactory("Windows.Graphics.Capture.GraphicsCaptureItem");
        ...
        /// <summary>
        /// Obtiene el activation factory WinRT de una clase activable vía RoGetActivationFactory
        /// (.NET Core no expone WindowsRuntimeMarshal.GetActivationFactory, que era de .NET Framework).
        /// </summary>
        private static IGraphicsCaptureItemInterop GetActivationFactory(string activationId)
        {
            IntPtr hstring = IntPtr.Zero;
            IntPtr factoryPointer = IntPtr.Zero;
            try
            {
                if (WindowsCreateString(activationId, activationId.Length, out hstring) != 0 || hstring == IntPtr.Zero)
                {
                    return null;
                }

                Guid iid = typeof(IGraphicsCaptureItemInterop).GUID;
                int hr = RoGetActivationFactory(hstring, ref iid, out factoryPointer);

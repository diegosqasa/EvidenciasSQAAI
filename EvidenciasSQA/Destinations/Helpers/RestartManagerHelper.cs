...
{
    /// <summary>
    /// Helper class for integrating EvidenciasSQA with the Windows Restart Manager.
    ...
    internal static class RestartManagerHelper
    {
        ...
        /// </summary>
        public static string StateDirectory => Path.Combine(Path.GetTempPath(), "EvidenciasSQA", "RestartState");

        /// <summary>
        /// Registers EvidenciasSQA for automatic restart by the Windows Restart Manager.
        /// When the Restart Manager restarts EvidenciasSQA, it will use the <c>--restore</c> argument
        /// so that EvidenciasSQA can restore any open image editors.
        /// </summary>
        public static void RegisterForRestart()
        {
            ...
            ApplicationRestartManager.ListenForEndSession(
                ...
                {
                    // Do the work, save state and exit EvidenciasSQA
        ...
        /// <summary>
        /// Unregisters EvidenciasSQA from automatic restart by the Windows Restart Manager.
        /// </summary>
        ...
        /// as <c>.evidenciassqa</c> files, so they can be restored after a Restart Manager restart.
        ...
        public static void SaveEditorState()
        {
            try
            {
                ...
                // Remove stale .evidenciassqa files from any previous session
                foreach (string oldFile in Directory.GetFiles(stateDir, "*.evidenciassqa"))
                {
                    try
                    {
                        File.Delete(oldFile);
                ...
                _dispatcher.Invoke(() =>
                {
                    foreach (var editor in editors)
                    {
                        try
                        {
                            string filename = Path.Combine(stateDir, editor.Surface.ID + ".evidenciassqa");
        ...
        /// <summary>
        /// Adds any <c>.evidenciassqa</c> state files saved by <see cref="SaveEditorState"/> to the
        /// supplied <paramref name="transport"/> as <see cref="CommandEnum.OpenFile"/> commands, so
        /// that the editors will be restored when EvidenciasSQA starts with the <c>--restore</c> argument.
        /// </summary>
        /// <param name="transport">Transport object to which restore commands are added.</param>
        public static void RestoreState()
        {
            Log.InfoFormat("EvidenciasSQA started with a request to restore state.");
            try
            {
                string stateDir = StateDirectory;
                ...
                foreach (string filePath in Directory.GetFiles(stateDir, "*.evidenciassqa"))
                {
                    _dispatcher.Invoke(() => {
                        ISurface surface = new Surface();
                        surface = ImageIO.LoadEvidenciasSQASurface(filePath, surface);

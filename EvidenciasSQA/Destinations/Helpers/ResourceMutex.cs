    ...
    public class ResourceMutex : IDisposable
    {
        ...
        public bool Lock()
        {
            ...
            try
            {
                ...
                // 1) Create Mutex
                // .NET 9: el ctor con MutexSecurity no existe (solo .NET Framework);
                // se crea con 3 argumentos y se aplica la ACL a posteriori (best-effort).
                ...
                if (_applicationMutex != null)
                {
                    ...
                    {
                        Log.DebugFormat("Could not set mutex security: {0}", e.Message);
                    }
                }

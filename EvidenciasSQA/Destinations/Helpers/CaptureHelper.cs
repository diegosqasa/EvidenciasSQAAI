...
namespace EvidenciasSQA.Helpers
{
    ...
    public class CaptureHelper : IDisposable
    {
        ...
        private void MakeCapture()
        {
            ...
            switch (_captureMode)
            {
                ...
                case CaptureMode.ActiveWindow:
                    Log.Debug("[MakeCapture] CaptureMode.ActiveWindow - calling CaptureActiveWindow()");
                    if (CaptureActiveWindow())
                    {
                        ...
                        _capture.CaptureDetails.Title = _selectedCaptureWindow?.Text ?? "Ventana activa";
                        Log.Debug("[MakeCapture] CaptureActiveWindow succeeded");
                    ...
                    {
                        Log.Warn("[MakeCapture] CaptureActiveWindow FAILED - no valid window to capture, cancelling capture");
                        // Show error to user and cancel capture (no fallback to FullScreen)
                        var uiContext = SimpleServiceProvider.Current.GetInstance<SynchronizationContext>();
                        uiContext?.Post(_ => 
                        {
                            MessageBox.Show(
                                Language.GetString("error_nowindowtocapture"),
                                Language.GetString("error"),
                                MessageBoxButtons.OK,
                                MessageBoxIcon.Warning);
                        }, null);
                        // Do not capture anything, just return
                    }
                    break;
                case CaptureMode.FullScreen:
                    ...
                    // Set title based on number of screens and which screen is captured
                    if (Screen.AllScreens.Length > 1)
                    {
                        // Identify screens based on Windows configuration
                        var screens = Screen.AllScreens;
                        var primaryScreen = Screen.PrimaryScreen;
                        var cursorPos = User32Api.GetCursorLocation();
                        
                        // Check if cursor is on a specific screen
                        var cursorScreen = screens.FirstOrDefault(s => s.Bounds.Contains(cursorPos));
                        
                        if (cursorScreen != null)
                        {
                            // Check if cursor is on primary screen
                            if (cursorScreen == primaryScreen)
                            {
                                _capture.CaptureDetails.Title = "Pantalla Principal";
                            }
                            else
                            {
                                // Find the index of the cursor screen vs primary screen
                                int primaryIndex = Array.IndexOf(screens, primaryScreen);
                                int cursorIndex = Array.IndexOf(screens, cursorScreen);
                                
                                // If cursor is on a screen after primary, mark as secondary
                                if (cursorIndex > primaryIndex)
                                {
                                    _capture.CaptureDetails.Title = "Pantalla Secundaria";
                                }
                                else
                                {
                                    _capture.CaptureDetails.Title = "Pantalla Principal";
                        ...
                        {
                            // Cursor not on any screen (unlikely), use primary screen title
                            _capture.CaptureDetails.Title = "Pantalla Principal";
                        }
                        
                        // Also check if we captured a specific screen by index
                        if (CoreConfig.ScreenToCapture > 0 && CoreConfig.ScreenToCapture <= screens.Length)
                        {
                            var capturedScreen = screens[CoreConfig.ScreenToCapture - 1];
                            _capture.CaptureDetails.Title = $"Pantalla {(CoreConfig.ScreenToCapture == 1 ? "Principal" : "Secundaria")}";
                        }
                    }
                    else
                    {
                        _capture.CaptureDetails.Title = "Captura de pantalla";
                    }
                ...
                case CaptureMode.File:
                    ...
                    {
                        // TODO: Fix that the EvidenciasSQA format needs a separate code path
                        try
                        {
                            if (filename.ToLower().EndsWith("." + OutputFormat.evidenciassqa))
                            {
                                ISurface surface = new Surface();
                                surface = ImageIO.LoadEvidenciasSQASurface(filename, surface);
        ...
        {
            // Direct save flow: ignore the configured destinations and always save the
            // capture directly to disk (PNG, fixed QA folder) without any picker/menu.
            IDestination destination = DestinationHelper.GetDestination(nameof(WellKnownDestinations.FileNoDialog));
            if (destination != null)
            {
                _capture.CaptureDetails.AddDestination(destination);
            }
        ...
        private void HandleCapture()
        {
            ...
            {
                // Direct save flow: the destination picker menu is disabled, captures are
                // always saved automatically to the QA evidence folder.
                captureDetails.CaptureDestinations.RemoveAll(d => d.Designation == nameof(WellKnownDestinations.Picker));
            }
                ...
                foreach (IDestination destination in captureDetails.CaptureDestinations.OrderBy(d => d.Priority).ThenBy(d => d.Description))
                {
                        ...
                        destination.Designation == nameof(WellKnownDestinations.FileDialog))
                    {
                        ...
                        else
                        {
                            ...
                            // Direct save flow: never show a dialog and never overwrite an existing
                            // capture. If the generated filename already exists, append a numeric suffix.
                            if (fullPath != null && File.Exists(fullPath))
                            {
                                ...
                                {
                                    fullPath = Path.Combine(directory, $"{fileNameWithoutExtension}_{suffix++}{extension}");
                                }
                            }
                        ...
                        var task = Task.Run(() =>
                        {
                            try
                            {
                                ...
                                // Header corporativo: se hornea sobre el PNG ya persistido
                                // (best-effort, idempotente) ANTES de notificar al visor, para
                                // que el visor SIEMPRE cargue la captura con cabecera.
                                // Mismo orden que FileDestination.ExportCapture: (A) Escritura →
                                // (B) Horneado síncrono → (C) Notificación al bus.
                                FileDestination.BakeCorporateHeader(bgFullPath, captureDetails);

                                // Punto único de notificación del flujo de persistencia: valida
                                // existencia del archivo y difunde CaptureSaved con la ruta real
                                // (nunca "el PNG más reciente de la carpeta").
                                new SqaCaptureFlow().OnCaptureCompleted(bgFullPath);
        ...
        private bool CaptureActiveWindow()
        {
            ...
            Log.Debug("[CaptureActiveWindow] INICIO - Capturing active window");
            if (_selectedCaptureWindow != null)
            {
                Log.Debug("[CaptureActiveWindow] Using supplied window");
                presupplied = true;
            }
            else
            {
                ...
                if (_selectedCaptureWindow != null)
                {
                    ...
                    {
                        Log.Debug("[CaptureActiveWindow] Capturing window: " + _selectedCaptureWindow.Text + " with " + _selectedCaptureWindow.WindowRectangle);
                    }
                }
                else
                {
                    Log.Warn("[CaptureActiveWindow] WindowDetails.GetActiveWindow() returned NULL!");
                }
            }

            if (_selectedCaptureWindow == null || (!presupplied && _selectedCaptureWindow.Iconic))
            {
                Log.Warn("[CaptureActiveWindow] No window to capture! _selectedCaptureWindow=" + _selectedCaptureWindow + ", Iconic=" + _selectedCaptureWindow?.Iconic);
            ...
            if (_selectedCaptureWindow == null)
            {
                Log.Warn("[CaptureActiveWindow] No window to capture, after SelectCaptureWindow!");
                // Nothing to capture, code up in the stack will capture the full screen
                return false;
            }
            ...
            Log.Debug("[CaptureActiveWindow] FIN - returnValue=" + returnValue);
            return returnValue;
        }
        ...
        public static ICapture CaptureWindow(WindowDetails windowToCapture, ICapture captureForWindow, WindowCaptureMode windowCaptureMode)
        {
            ...
            using (Process process = windowToCapture.Process)
            {
                ...
                if (isAutoMode)
                {
                    ...
                    // In https://github.com/evidenciassqa/evidenciassqa/issues/373 it was shown that PrintWindow (GDI) works great with Windows 11
                    // In https://github.com/evidenciassqa/evidenciassqa/issues/658 is was made clear it DOESN'T!
        ...
        private void SetDpi()
        {
            ...
            var mainForm = SimpleServiceProvider.Current.GetInstance<IEvidenciasSQAMainForm>();
